...
public class VersionProvider1_8_R1 implements NMSProvider {
    ...
    public void rickroll(Player player, @Nullable CommandSender sender) {
        try {
            ...
            PacketPlayOutNamedSoundEffect packet = new PacketPlayOutNamedSoundEffect("music.rickroll",
                    player.getEyeLocation().getX(), player.getEyeLocation().getY(),
            ...
            EntityPlayer npc = new EntityPlayer(((CraftPlayer) player).getHandle().server,
                    ((CraftPlayer) player).getHandle().u(), RickrollCommand.rickastley,
                    new PlayerInteractManager(((CraftPlayer) player).getHandle().u()));
