    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            if (sender.hasPermission("essentials.gamemode")) {
                ...
                player.sendMessage(CC.translate("&6Gamemode: &aSpectator"));
            }
        } else {
            if (sender.hasPermission("essentials.gamemode.others")) {
                ...
                target.sendMessage(CC.translate("&6Gamemode: &aSpectator"));
                sender.sendMessage(CC.translate("&7" + target.getName() + "&6 " + (target.getName().endsWith("s") ? "'" : "'s") + "&6gamemode: &aSpectator"));
            }
        }
        return true;
    }
