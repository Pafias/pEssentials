...
public class SoundCommand extends ICommand {
    ...
    public List<String> tabHandler(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 1) return Arrays.asList("play", "stop");
            ...
            list.add("all");
            list.addAll(plugin.getServer().getOnlinePlayers().stream().map(Player::getName).filter(p -> p.toLowerCase().startsWith(args[1].toLowerCase())).collect(Collectors.toList()));
            return list;
        } else if (args.length == 3 && args[0].equalsIgnoreCase("play"))
            return Arrays.stream(Sound.values()).map(Sound::name).filter(s -> s.toLowerCase().startsWith(args[2].toLowerCase())).collect(Collectors.toList());
