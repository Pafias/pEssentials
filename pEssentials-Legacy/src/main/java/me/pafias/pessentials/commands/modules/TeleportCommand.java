...
public class TeleportCommand extends ICommand {
    ...
    public void commandHandler(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0)
            sender.sendMessage(LCC.t("&c/" + label + " <player> [player]"));
            ...
            sender.sendMessage(LCC.t("&6Teleported to &d" + target.getName()));
            ...
            if (player1 == null || player2 == null || (sender instanceof Player && (!((Player) sender).canSee(player1.getPlayer()) || !((Player) sender).canSee(player2.getPlayer())))) {
                sender.sendMessage(LCC.t("&cPlayer not found!"));
                return;
            }
            player1.getPlayer().teleport(player2.getPlayer());
            sender.sendMessage(LCC.t("&6Teleported &d" + player1.getName() + " &6to &d" + player2.getName()));
            ...
            try {
                ...
                sender.sendMessage(LCC.t("&cInvalid coordinates."));
                return;
            }
            ((Player) sender).teleport(new Location(((Player) sender).getWorld(), x, y, z));
            sender.sendMessage(LCC.tf("&6Teleported to &d%f.2, %f.2, %f.2", x, y, z));
        }
    }
