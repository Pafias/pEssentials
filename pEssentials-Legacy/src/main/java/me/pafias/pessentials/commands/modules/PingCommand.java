...
public class PingCommand extends ICommand {
    ...
    public void commandHandler(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 && sender instanceof Player) {
            ...
            sender.sendMessage(LCC.t("&cYou don't have permission to check other players' ping ;("));
            return;
        }
        ...
        if (target == sender)
            pingMsg = LCC.t("&6Ping: " + pingColored + "&7ms");
        else
            pingMsg = LCC.t("&d" + target.getName() + (target.getName().endsWith("s") ? "&6'" : "&6's") + " &6ping: " + pingColored + "&7ms");
