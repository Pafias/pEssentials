...
public class SpeedCommand extends ICommand {
    ...
    public void commandHandler(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(LCC.t("&6Currently, your walking speed is &7" + player.getWalkSpeed() + " &6and your flying speed is &7" + player.getFlySpeed()));
            sender.sendMessage(LCC.t("&6Use /speed <speed> to change it"));
            ...
            if (flying) {
                ...
                player.sendMessage(LCC.t("&6Your fly speed multiplier is now &7" + speed));
            } else {
                player.setWalkSpeed(getRealMoveSpeed(speed, flying));
                player.sendMessage(LCC.t("&6Your walk speed multiplier is now &7" + speed));
            }
        } else {
            if (sender.hasPermission(getPermission() + ".others")) {
                ...
                if (flying) {
                    ...
                    if (!Arrays.asList(args).contains("-s"))
                        target.sendMessage(LCC.t("&6Your fly speed multiplier is now &7" + speed));
                    sender.sendMessage(LCC.t("&6Changed players' fly speed multiplier to &7" + speed));
                } else {
                    target.setWalkSpeed(getRealMoveSpeed(speed, flying));
                    if (!Arrays.asList(args).contains("-s"))
                        target.sendMessage(LCC.t("&6Your walk speed multiplier is now &7" + speed));
                    sender.sendMessage(LCC.t("&6Changed players' walk speed multiplier to &7" + speed));
                }
            }
        }
    }
