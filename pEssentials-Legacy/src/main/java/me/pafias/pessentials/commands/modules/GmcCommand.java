...
public class GmcCommand extends ICommand {
    ...
    public void commandHandler(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            player.sendMessage(LCC.t("&6Gamemode: &acreative"));
        } else {
            ...
            if (args[0].equalsIgnoreCase("@a") || args[0].equalsIgnoreCase("*"))
                plugin.getServer().getOnlinePlayers().forEach(p -> {
                    ...
                    if (!silent)
                        p.sendMessage(LCC.t("&6Gamemode: &acreative"));
                });
            else {
                ...
                if (!silent)
                    target.sendMessage(LCC.t("&6Gamemode: &acreative"));
                sender.sendMessage(LCC.tf("&6Gamemode for &7%s&6: &acreative", target.getName()));
            }
        }
        return;
    }
